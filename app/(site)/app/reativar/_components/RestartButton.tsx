...
// possible, so that case gets its own message + a link to /app/billing instead
// of the generic error line. Never assumes success client-side — onRestarted
// only fires after a real 200 from the backend.
    ...
    <div>
      ...
      {checkoutRequired && (
        <div role="alert" className="rtv-payment-notice">
          <p style={{ margin: 0 }}>
            Sua assinatura precisa passar por um novo checkout antes de reiniciar o teste.
          </p>
          <Link href="/app/billing" className="btn btn--outline btn--sm mt-s">
            Ir para Assinatura
          </Link>
        </div>
      )}
