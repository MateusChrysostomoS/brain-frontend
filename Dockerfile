...
#   clínica não está configurada neste ambiente" banner and keeps every secretarIA
#   page on demo data. Setting this in EasyPanel's Environment panel has NO effect:
#   the build is a static export served by nginx, so NEXT_PUBLIC_* values only exist
